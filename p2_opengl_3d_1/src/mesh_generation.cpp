...
{
    auto parametric_surface = [parametric_line](double t, double r, bool mode)
    {
        ...
        if(mode == true)
        {
            ...
            p.y *= (pow(sin((10*r + 0.5) * 10 * glm::two_pi<double>()) ,6) + 3) / 3;
    ...
    for (int r = 0; r < rotation_segments; ++r)
        for (int v = 0; v < vertical_segments; ++v)
            positions.push_back(parametric_surface(v / double(vertical_segments - 1), (r / double(rotation_segments)),mode));
    ...
    for (int r = 0; r < rotation_segments; ++r)
        for (int v = 0; v < vertical_segments; ++v)
        {
            ...
            auto to_next_v = parametric_surface(nv + epsilonv, nr,mode) - parametric_surface(nv, nr,mode);
            auto from_prev_v = parametric_surface(nv, nr,mode) - parametric_surface(nv - epsilonv, nr,mode);
            auto tangent_v = (to_next_v + from_prev_v) / 2.;

            auto to_next_r = parametric_surface(nv, nr + epsilonr,mode) - parametric_surface(nv, nr,mode);
            auto from_prev_r = parametric_surface(nv, nr,mode) - parametric_surface(nv, nr - epsilonr,mode);
...
glm::dvec2 ParametricExperiment(double t)
{
    // [0, 1]
    t -= 0.5;
    // [-0.5, 0.5]
    ...
    double r = (1+cos(t))*(1+0.3*cos(0.5*t))*(0.9+0.1*cos(20*t));
    r /= 2.3;
    
    return glm::dvec2(r*cos(t),r*sin(t));
}
